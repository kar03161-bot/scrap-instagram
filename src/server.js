...
function hasReadableInstagramCookieFile() {
  try {
    ...
    return (
      ...
      parsed.some(
        (c) => c && c.name === "sessionid" && String(c.value).length > 0,
      )
    );
  } catch {
    return false;
...
async function dbQuery(strings, ...values) {
  ...
  dbPool ||= new Pool({
    connectionString:
      postgresUrlWithExplicitVerifyFull(databaseUrl) ?? databaseUrl,
    max: poolMax,
...
app.post("/api/influencers/check", async (req, res) => {
  ...
  if (list.length === 0) {
    return res
      .status(400)
      .json({ error: "조회할 인스타그램 아이디를 하나 이상 입력해 주세요." });
  }
...
app.post("/api/analyze", async (req, res) => {
  ...
  if (list.length === 0) {
    ...
    return res
      .status(400)
      .json({ error: "분석할 인스타그램 아이디를 하나 이상 입력해 주세요." });
  }
  ...
  if (list.length > 25) {
    ...
    return res
      .status(400)
      .json({ error: "한 번에 최대 25개 계정까지 분석할 수 있습니다." });
  }
  ...
  try {
    ...
    if (existing.length > 0) {
      ...
      return res
        .status(409)
        .json({ error: "이미 분석한 계정입니다", existing });
    }
...
if (!process.env.VERCEL) {
  app.listen(PORT, () => {
    ...
    if (headless) {
      ...
      console.log(
        "[안내] Chrome 창이 열립니다. Instagram 보안·동의 화면은 브라우저에서 처리할 수 있습니다.",
      );
    }
    if (
      !hasReadableInstagramCookieFile() &&
